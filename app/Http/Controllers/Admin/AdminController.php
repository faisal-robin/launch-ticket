...
//        print_r($company_info);die;
        ...
        if ($company_info != NULL) {
            if ($request->hasFile('company_image')) {
                if (File::exists('storage/app/' . $company_info->company_image)) {
                    File::delete('storage/app/' . $company_info->company_image);
                }
...
//     End   Company Thumbnail
        ...
        if ($company_info != NULL) {
            DB::table('company_info')->where('id', $company_info->id)->update($data);
        }
        if ($company_info == NULL) {
